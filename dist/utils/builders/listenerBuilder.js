...
export const createListener = (name, event, execute, options = {}) => {
    const { ignoreBots = true, ignoreSelf = true, channels: channelKeys, ignoreChannels, users, ignoreUsers, roles, ignoreRoles, } = options;
    const filterFunction = (...args) => {
        ...
        if (typeof context === "object" &&
            ...
            if (channelKeys?.length) {
                const requiredChannelIds = channelKeys.map((key) => config.channels[key]);
