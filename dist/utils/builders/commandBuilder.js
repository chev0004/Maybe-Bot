 ...
 */
export const createCommand = (name, description, execute, { ownerOnly = false, adminOnly = false, guildOnly = true, setup = (builder) => builder, allowedChannels = [], } = {}) => {
    ...
    return {
        ...
        async execute(interaction, client, options) {
            ...
            if (allowedChannels.length > 0) {
                const requiredChannelIds = allowedChannels.map((key) => config.channels[key]);
