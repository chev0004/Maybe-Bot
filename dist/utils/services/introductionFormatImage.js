...
function setFont(ctx, forLatin = false) {
    ctx.font = forLatin ? FONT_STRING_LATIN : FONT_STRING_JP;
}
function drawRawText(ctx, text, x, y, _maxWidth) {
    if (text.length === 0)
        return x;
    let currentX = x;
    let segment = "";
...
function measureRawText(ctx, text) {
    if (text.length === 0)
        return 0;
    let width = 0;
    let segment = "";
...
function matchUserLinesToHeaders(content) {
    const lines = content.split("\n");
    const headerToUserLine = [
        null,
        null,
        null,
        null,
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let h = 0; h < CORRECT_TEMPLATE_HEADERS.length; h++) {
            if (headerToUserLine[h] !== null)
                continue;
            const header = CORRECT_TEMPLATE_HEADERS[h];
            const startsWithHeader = trimmed.startsWith(header);
            const lineIsPrefixOfHeader = header.startsWith(trimmed) && trimmed.length >= 2;
            const prefixLen = commonPrefixLength(trimmed, header);
            const typoMatch = prefixLen >= MIN_PREFIX_MATCH_LEN && prefixLen > bestPrefix;
            if (startsWithHeader) {
                bestH = h;
    ...
    const leadingLines = assignedLineIndices.length > 0
        ? lines.slice(0, Math.min(...assignedLineIndices))
        : lines;
    const trailingLines = assignedLineIndices.length > 0
        ? lines.slice(Math.max(...assignedLineIndices) + 1)
        : [];
    return { headerToUserLine, leadingLines, trailingLines };
}
function getWrongRangesForHeader(headerIndex, userLine) {
    const expected = CORRECT_TEMPLATE_HEADERS[headerIndex];
    let firstWrong = 0;
    while (firstWrong < userLine.length &&
        firstWrong < expected.length &&
        userLine[firstWrong] === expected[firstWrong]) {
        firstWrong++;
    }
    const noError = firstWrong === userLine.length && firstWrong === expected.length;
    if (noError)
        return [];
    const errorAtEnd = firstWrong === userLine.length && userLine.length < expected.length;
    if (errorAtEnd) {
        return [[0, userLine.length]];
    }
    return [[firstWrong, userLine.length]];
}
export function drawUserMessageWithHighlights(content) {
    const { headerToUserLine, leadingLines, trailingLines } = matchUserLinesToHeaders(content);
    const canvas = createCanvas(MAX_CANVAS_WIDTH, 100);
    const ctx = canvas.getContext("2d");
    const linesToMeasure = [...leadingLines, ...trailingLines];
    ...
    const textWidth = Math.min(MAX_CANVAS_WIDTH - PADDING * 2, Math.ceil(Math.max(...linesToMeasure.map((line) => measureRawText(ctx, line)), measureRawText(ctx, " "))) + 1);
    ...
    const firstSlotHasTypoNoLeading = leadingLines.length === 0 &&
    ...
    for (let i = 0; i < CORRECT_TEMPLATE_HEADERS.length; i++) {
        ...
        for (const [start, end] of ranges) {
            const xStart = PADDING + measureRawText(rctx, userLine.substring(0, start));
...
export function drawCorrectTemplate() {
    ...
    textWidth = Math.min(MAX_CANVAS_WIDTH - PADDING * 2, Math.ceil(textWidth) + 1);
    const width = textWidth + PADDING * 2;
