...
const isTextInEmbed = (embed, text) => {
    ...
    if (embed.fields) {
        for (const field of embed.fields) {
            if (field.name?.toLowerCase().includes(searchText))
                return true;
            if (field.value?.toLowerCase().includes(searchText))
                return true;
        }
    }
    return false;
 ...
 */
const logBump = async (userId, username) => {
 ...
 */
export const handleBump = async (message, client, bumpSource, bumpIdentifierText) => {
    if (!isTestInstance &&
    ...
    if (!isTextInEmbed(embed, bumpIdentifierText))
        return;
