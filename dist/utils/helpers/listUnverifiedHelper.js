...
const formatTenureSinceJoin = (joinedTimestamp) => {
...
export const generatePage = (memberArray, sortCriteria, sortOrder, currentPage, isTestMode = false) => {
    ...
    const listContent = currentItems.map(formatMemberLine).join("\n") ||
        "このページにメンバーはいません。";
    ...
    const embed = new EmbedBuilder()
        .setTitle(title)
        .setDescription(`**${criteriaLabel}** を **${sortLabel}** で表示。\n\n${listContent}`)
    ...
    const buttonRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
        .setCustomId(`listunverified_prev_${currentPage}_${sortCriteria}_${sortOrder}_${testModeFlag}`)
        .setLabel("◀")
        .setStyle(ButtonStyle.Primary)
        .setDisabled(currentPage === 0), new ButtonBuilder()
        .setCustomId(`listunverified_sort_${currentPage}_${sortCriteria}_${sortOrder}_${testModeFlag}`)
        .setLabel(sortOrder === "asc" ? "昇順 ▲" : "降順 ▼")
        .setStyle(ButtonStyle.Secondary), new ButtonBuilder()
        .setCustomId(`listunverified_next_${currentPage}_${sortCriteria}_${sortOrder}_${testModeFlag}`)
        .setLabel("▶")
        .setStyle(ButtonStyle.Primary)
        .setDisabled(currentPage >= totalPages - 1));
    const selectMenuRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
        .setCustomId(`listunverified_select_${currentPage}_${sortOrder}_${testModeFlag}`)
