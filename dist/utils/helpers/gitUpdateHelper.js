...
const isInDist = (path) => path.startsWith("dist/") || path.includes("/dist/");
const mergeCommitRegex = /^Merge (pull request|PR) #(\d+) from (\S+)|Merge branch '(\S+)'/;
/**
...
export const parseGitUpdateOutput = (commitLog, gitStdout, gitStderr) => {
    ...
    for (const line of outputLines) {
        ...
        if (fileChangeMatch) {
            ...
            if (renameMatch) {
                ...
                if (isInDist(fromPath) || isInDist(toPath))
                    continue;
                formattedFileLines.push(`${setColor("dimYellow", "rn:")} ${fromPath}`, `${setColor("dimYellow", "to:")} ${toPath}`);
            }
            else {
                if (isInDist(filePath))
                    continue;
    ...
    const finalChanges = changes.trim() ||
