...
export const handleApprovalProcess = async (interaction, requiredApprovals, actionMessage, actionMessageEN, successCallback, failureMessage) => {
    await interaction.deferReply();
    try {
        const pollMessage = await interaction.editReply(`${actionMessage}には${requiredApprovals}つの承認が必要です。\n✅を押して承認してください。\nWe need ${requiredApprovals} ${requiredApprovals === 1 ? "approval" : "approvals"} to ${actionMessageEN}.\nReact with ✅ to approve.`);
