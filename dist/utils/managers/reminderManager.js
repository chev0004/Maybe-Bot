...
import { addReminder, getReminders, removeReminderById, } from "./dataManager.js";
...
const executeReminder = async (reminder, client) => {
    try {
        ...
        if (!channel || !channel.isTextBased()) {
            console.warn(`Could not find channel ${reminder.channelId} for reminder, or it's not a text channel.`);
        ...
        const embedMsg = new EmbedBuilder()
            ...
            .setTitle(title)
            .setDescription(`2時間経ちました。もう一度 **${bumpSource}** の \`\`/bump\`\` が出来るようになりました！`)
            .setTimestamp();
        await channel.send({
...
export const scheduleReminder = async (reminderDetails, client) => {
    ...
    const existingReminder = reminders.find((r) => r.source === reminderDetails.source);
    if (existingReminder) {
