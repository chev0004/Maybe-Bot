                ...
                if (fs.statSync(categoryPath).isDirectory()) {
                    ...
                    for (const file of listenerFiles) {
                        ...
                        if (listener.name && listener.execute && listener.event) {
                            ...
                            const listenerCategory = category.charAt(0).toUpperCase() + category.slice(1);
                            console.log(`Loaded [${listenerCategory}] listener: ${listener.name} for event ${listener.event}`);
                        }
                        else {
                            console.log(`[WARNING] Listener at ${file} is missing required properties.`);
                        }
                    }
                }
            }
