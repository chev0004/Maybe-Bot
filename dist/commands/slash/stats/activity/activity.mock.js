...
const scaleActivityData = (data, factor) => {
    const scaledHourly = data.hourlyActivity.map((v) => Math.round(v * factor * (0.9 + Math.random() * 0.2)));
    if ("totalMessages" in data) {
        return {
            ...data,
            hourlyActivity: scaledHourly,
            totalMessages: Math.round(data.totalMessages * factor),
            averageParticipants: Math.round(data.averageParticipants * (0.8 + Math.random() * 0.4)),
        };
    }
    return {
        ...data,
