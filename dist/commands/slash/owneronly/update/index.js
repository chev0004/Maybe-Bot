...
const inlineCode = (text) => `\`${text.replace(/`/g, "'")}\``;
export default createCommand("update", "GitHubから最新のコミットを取得し、BOTを再起動する。Pulls the latest changes from GitHub and restarts the bot.", async (interaction) => {
    ...
    try {
        try {
            ...
            embed
                .setColor(Colors.red)
                .setDescription("Gitリポジトリ内で実行されていません。BOTをリポジトリのルートから起動してください。");
            await interaction.editReply({ embeds: [embed] });
            return;
        }
        ...
        try {
            ...
            embed
                .setColor(Colors.red)
                .setDescription(`リモートブランチ ${inlineCode(`origin/${PULLED_BRANCH}`)} が見つかりません。${inlineCode("git fetch origin")} を確認してください。`);
            await interaction.editReply({ embeds: [embed] });
            return;
        }
        ...
        if (!isForceMode && !commitLog && !needsNpmInstall) {
            embed
                ...
                .setDescription(`BOTは既に最新の状態です（${inlineCode(PULLED_BRANCH)}）。`);
            await interaction.editReply({ embeds: [embed] });
            return;
        }
        if (!isForceMode) {
        ...
        const { stdout: headBeforeStdout } = await execPromise("git rev-parse HEAD");
        ...
        try {
            ...
            const hasConflict = /conflict|CONFLICT|merge conflict/i.test(stderr) ||
                /conflict|CONFLICT/i.test(err.message);
            embed
                .setColor(Colors.red)
                .setDescription(hasConflict
        ...
        let statStdout = "";
        try {
            const { stdout: diffStatOut } = await execPromise(`git diff --stat=300 --no-color ${headBefore} HEAD -- . ':(exclude)dist' ':(exclude)dist/**'`);
            statStdout = diffStatOut;
        }
        catch (statErr) {
            console.warn("[Logger] git diff --stat failed:", statErr);
        }
        const { changes, files, repo } = parseGitUpdateOutput(commitLog, statStdout || pullStdout, fetchStderr || pullStderr);
        embed
        ...
        try {
            const { stdout: diffNames } = await execPromise(`git diff --name-only ${headBefore} HEAD`);
            const changedPaths = diffNames.split("\n").map((s) => s.trim());
            const distTouched = changedPaths.some((p) => p === "dist" || p.startsWith("dist/"));
        ...
        if (needsNpmInstall) {
            ...
            try {
                ...
                const addedMatch = installStdout.match(/(\d+)\s+packages? added/) ??
    ...
    setup: (builder) => {
        builder
            ...
            .setRequired(false)
            .addChoices({ name: "Normal Update", value: "normal" }, { name: "Rename Update", value: "rename" }, { name: "Dependencies Update", value: "npm" }));
        return builder;
    },
});
