...
const sendVoiceChannelCreationLog = async (interaction, client, channel, emoji, jpName, enName, limit) => {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`${member} が一時的なボイスチャンネル <#${channel.id}> を作成しました。`)
...
const sendVoiceChannelDeletionLog = async (_, client, channel) => {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`一時的なボイスチャンネル **${channel.name}** が非アクティブのため削除されました。`)
        .setColor(Colors.red)
        .setTimestamp();
    await logChannel.send({ embeds: [embed] });
};
...
export default createCommand("vc", "一時的なボイスチャンネルを作成する。Create a temporary voice channel.", async (interaction, client, _options) => {
    try {
        ...
        await sendVoiceChannelCreationLog(interaction, client, channel.isVoiceBased()
            ? channel
            : (await interaction.guild.channels.fetch(channel.id)), emoji, jpName, formattedEnName, limit);
            ...
            if (members && members.size === 0) {
                setTimeout(async () => {
                    ...
                    if (currentVC &&
                        ...
                        await sendVoiceChannelDeletionLog(interaction, client, currentVC);
        ...
        const jpSuccessMessage = `(${limit === 0 ? "無制限" : `${limit}人制限`}) 一時的なボイスチャンネル ${channel} を作成しました！`;
        const enSuccessMessage = `(${limit === 0 ? "No limit" : `Max ${limit} people`}) Created temporary voice channel ${channel}!`;
    ...
    setup: (builder) => {
        builder
            ...
            .addStringOption((option) => option
            .setName("jpname")
            .setDescription("ボイスチャンネルの名前 (日本語)。Name of the voice channel (Japanese).")
            .setRequired(true))
            .addStringOption((option) => option
            .setName("enname")
            .setDescription("ボイスチャンネルの名前 (英語)。Name of the voice channel (English).")
            .setRequired(true))
            .addIntegerOption((option) => option
            .setName("limit")
            .setDescription("ユーザー制限 (0は無制限)。User limit (0 for unlimited).")
            .setRequired(true)
            .setMinValue(0)
            .setMaxValue(99));
        return builder;
