...
export default createCommand("whitelist", "マイクラサーバーのホワイトリストにユーザーを追加する。Adds a user to the Minecraft server whitelist.", async (interaction, _client, options) => {
    ...
    try {
        ...
        embed
            ...
            .setDescription(`ユーザー「${usernameToWhitelist}」をホワイトリストに追加しました。`)
            .addFields({
            name: "プラットフォーム",
        ...
        console.error(`Error adding user ${usernameToWhitelist} to whitelist:`, error);
        const errorMessage = parseExarotonError(error);
        embed
            .setColor(Colors.red)
            .setDescription(`ユーザー「${usernameToWhitelist}」のホワイトリスト追加に失敗しました。`)
            .addFields({
            name: "エラー詳細",
    ...
    setup: (builder) => {
        builder
            ...
            .setName("username")
            .setDescription("ホワイトリストに追加するユーザー名。The username to add to the whitelist.")
            .setRequired(true))
            .addBooleanOption((option) => option
            .setName("bedrock")
            .setDescription("統合版ユーザーの場合はtrueを選択。Set to true if the user is a Bedrock player.")
            .setRequired(false));
        return builder;
    },
});
