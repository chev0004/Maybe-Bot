import { ActionRowBuilder, ApplicationCommandType, ButtonBuilder, ButtonStyle, ComponentType, EmbedBuilder, MessageFlags, PermissionsBitField, } from "discord.js";
...
export default createMenuCommand("Purge to here", ApplicationCommandType.Message, async (interaction, _client, _options) => {
    ...
    try {
        ...
        const messageArray = Array.from(fetchedMessages.values()).sort((a, b) => b.createdTimestamp - a.createdTimestamp);
        const targetIndex = messageArray.findIndex((msg) => msg.id === targetMessage.id);
        if (targetIndex === -1) {
            await interaction.editReply({
                content: "選択されたメッセージは直近100件のメッセージ内に見つかりませんでした。\nCould not find the target message within the last 100 messages.",
            });
            return;
        }
    ...
    if (messagesToDelete.length === 0) {
        const noMessagesEmbed = new EmbedBuilder()
            ...
            .setDescription("削除対象のメッセージが見つかりませんでした。\nNo messages were found to delete.");
        if (tooOldCount > 0) {
    ...
    const confirmationEmbed = new EmbedBuilder()
        ...
        .setDescription(`選択されたメッセージまで${messagesToDelete.length}件のメッセージを削除してもよろしいですか？\n` +
        `Are you sure you want to delete ${messagesToDelete.length} message(s) up to the selected message?`);
    if (tooOldCount > 0) {
        confirmationEmbed.addFields({
            name: "注意 / Warning",
            value: `${tooOldCount}件のメッセージは14日以上経過しているため削除できません。\n` +
    ...
    try {
        ...
        const timedOutRow = new ActionRowBuilder().addComponents(confirmButton.setDisabled(true), cancelButton.setDisabled(true));
        await interaction.editReply({
