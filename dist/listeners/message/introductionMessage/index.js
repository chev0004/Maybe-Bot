      ...
      if (
        ...
        if (roleIdToAssign) {
          ...
          if (!roleToAssign) {
            ...
            console.warn(
              `introductionMessage listener: Bot's highest role (${botMember.roles.highest.name}) is not high enough to assign language role '${roleToAssign.name}' (position ${roleToAssign.position}). Bot role position: ${botMember.roles.highest.position}.`,
            );
          } else {
