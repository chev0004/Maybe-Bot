 ...
 * @param {import('discord.js').ApplicationCommandType} type The type of the context menu command (Message or User).
 ...
 * @param {import('discord.js').PermissionResolvable[]} [options.requiredPermissions=[]] Specific permissions a user must have to run the command.
  ...
  return {
    ...
    async execute(interaction, client, options) {
      ...
      if (interaction.inGuild()) {
        ...
        if (requiredPermissions.length > 0) {
          const missingPerms =
            interaction.member.permissions.missing(requiredPermissions);
