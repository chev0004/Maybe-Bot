...
import { getData, saveData } from "../../../utils/dataManager.js"; // Import data manager
...
function colorizeGitOutput(text, branchToHighlight) {
  ...
  const branchPattern = branchToHighlight ? branchToHighlight.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") : null;
  coloredText = coloredText.replace(/^((?:[^|\n])+?\s*\|\s*\d+\s*)[+-]+$/gm, "$1");
  coloredText = coloredText.replace(/^(\s*\*\s*branch\s+)([a-zA-Z0-9_\-\/]+)((?:\s*->\s*.+)?|\s*\(.+\)|\s+[0-9a-fA-F]{7,}\s+.+)?/gm, (_, p1, p2, p3) => {
    const refinedP1 = p1.replace(/\s+$/, " ");
    const refinedP3 = p3 ? p3.replace(/^\s+/, " ").replace(/\s+$/, "") : "";
    return `${refinedP1}\u001b[2;34m${p2}\u001b[0m${refinedP3 || ""}`;
  });
  coloredText = coloredText.replace(/(\d+)\s+insertions?\(\+\)/g, "\u001b[2;36m+\u001b[0m" + "\u001b[2;36m$1\u001b[0m");
  coloredText = coloredText.replace(/(\d+)\s+deletions?\(\-\)/g, "\u001b[2;31m-\u001b[0m" + "\u001b[2;31m$1\u001b[0m");
  if (branchPattern) {
    coloredText = coloredText.replace(new RegExp(`(\\b${branchPattern}\\b)(\\s*->)`, "g"), `\u001b[2;34m$1\u001b[0m ->`);
    coloredText = coloredText.replace(new RegExp(`([a-zA-Z0-9_\\-\\/]+\\/)(${branchPattern})(\\b|$)`, "gm"), `$1\u001b[2;34m$2\u001b[0m$3`);
  }
  return coloredText;
}
...
export default {
  ...
  async execute(interaction) {
    ...
    if (isTestMode) {
      ...
      const embed = new EmbedBuilder().setTitle("BOTの更新 (テストモード)").setColor(Colors.yellow).setDescription("テスト用のGitプルシミュレーション中...").setTimestamp().setFooter({ text: "これはテスト実行です。BOTは実際の更新や再起動を行いません。" });
    ...
    try {
      ...
      if (commitLog) {
        embed.addFields({ name: "更新内容 / Changes", value: `\`\`\`\n${commitLog.substring(0, COMMIT_LOG_MAX_LEN)}\n\`\`\`` });
      ...
      await saveData();
      console.log(`Restart info saved to bot_data.json`);
      ...
      embed.setColor(Colors.red).setDescription("更新プロセス中にエラーが発生しました。").setFooter({ text: "BOTの更新に失敗しました。" });
      if (error.stdout) embed.addFields({ name: "Error (stdout)", value: `\`\`\`\n${error.stdout.substring(0, 1000)}\n\`\`\`` });
      if (error.stderr) embed.addFields({ name: "Error (stderr)", value: `\`\`\`\n${error.stderr.substring(0, 1000)}\n\`\`\`` });
      await interaction.editReply({ embeds: [embed] });
    }
  },
};